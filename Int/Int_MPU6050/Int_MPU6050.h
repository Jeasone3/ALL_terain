...
/* 7 位地址：AD0 接低为 0x68，接高时将此值改为 0x69。 */
#define MPU6050_ADDR 0x68U
/* 软件 IIC 发送的地址字节由 7 位地址自动生成。 */
...
/* 成功返回 1；设备不存在、IIC 错误或复位超时时返回 0。 */
uint8_t Int_MPU6050_Init(void);

/* 返回有符号 ADC 原始值；读取失败时不修改输出结构体。 */
uint8_t Int_MPU6050_Get_Gyro(Gyro_struct *gyro);
uint8_t Int_MPU6050_Get_Accel(Accel_struct *acc);
/* 连续读取 14 字节，保证六轴数据来自同一采样帧。 */
